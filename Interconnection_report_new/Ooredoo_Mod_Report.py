...
exeSql = '''
create table tmp_ooredoo_mod_report as
SELECT PK.PK_STIME,
...
AND SUBSTR(M.ORG_NUM, 0, 5) in ( '95996','95997','95995') and ( substr(m.term_num,0,5) in ('95939'))
...
/*+parallel(m, 8) */
FROM RECON_IB_CDR_67 M
WHERE SUBSTR(M.ORG_NUM, 0, 5) in ( '95996','95997','95995') and ( substr(m.term_num,0,5) in ('95939'))
and substr(m.term_num,0,6)<>'959593'
AND M.REID >= 1253
...
print "==========================tmp_ooredoo_mod_report finished =========================="
curs.close()
conn.close()
