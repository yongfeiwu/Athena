...
exeSql = '''
create table tmp_mpt_mec_report as
SELECT PK.PK_STIME,
  ...
  from (select TRUNC(t.START_TIME) PK_STIME, SUM(t.DURATION) PK_DUR
         ...
         where t.re_id = 1128
           ...
           and (substr(t.called_nbr, 0, 5) in
               ('95930', '95931', '95932', '95933', '95934', '95936') or
               substr(t.called_nbr, 0, 6) in ('959340', '959346', '959349'))
...
print "==========================tmp_mpt_mec_report finished =========================="
curs.close()
conn.close()
